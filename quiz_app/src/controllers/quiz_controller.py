...
def create_quiz():
    ...
    # TODO: Initialize an instance of QuizService (Hint: service =
    # QuizService())
    service = QuizService()

    # INCOMPLETE: Retrieve JSON data from the request
    # TODO: Get JSON data from the request using request.json and assign it to
    # `data`
    data = request.json

    # INCOMPLETE: Call the create_quiz method in the service
    # TODO: Use the service to create a quiz with the `data` and store the
    # returned quiz ID in `quiz_id`
    ...
    return (
        jsonify({"message": "Quiz created successfully", "quiz_id": quiz_id}),
        201,
    )
...
def get_quiz(quiz_id):
    ...
    # TODO: Call the `get_quiz` method with `quiz_id` and store the result in
    # `quiz`
...
def submit_quiz(quiz_id):
    ...
    # TODO: Get the answers from the request using
    # `request.json.get('answers')` and store in `user_answers`
    user_answers = request.json.get("answers")

    # INCOMPLETE: Use the service to evaluate the quiz with the provided
    # answers
    ...
    # TODO: If `score` is None, return an error with status 404. Otherwise,
    # return `score` and `message` with status 200.
    if score is None:
