...
def test_create_quiz(mock_create_quiz, client):
    ...
    # TODO: Use `client.post` to send a POST request to `/api/quizzes` with
    # JSON data
    url = "/api/quizzes"
    data = {
        "title": "Sample Quiz",
    ...
    # TODO: Assert that status code is 201, `quiz_id` in response is 1, and
...
def test_get_quiz(mock_get_quiz, client):
    ...
    # TODO: Create a MagicMock named `mock_quiz`, set `title` to "Sample
    # Quiz", and `questions` to a sample list

    mock_quiz = MagicMock()
    mock_quiz.title = "Sample Quiz"
    mock_quiz.questions = [{"text": "What is 2 + 2?", "answer": "4"}]
    ...
    # INCOMPLETE: Write assertions to check the response
    # TODO: Assert that status code is 200, `title` in response is "Sample
    # Quiz", and `mock_get_quiz` was called once
...
def test_submit_quiz(mock_evaluate_quiz, client):
    ...
    # TODO: Set `mock_evaluate_quiz.return_value` to (1, "Quiz evaluated
    # successfully")
    ...
    url = "/api/quizzes/1/submit"
    data = {"answers": ["4", "Toronto"]}
    response = client.post(url, json=data)

    # INCOMPLETE: Write assertions to check the response
    # TODO: Assert that status code is 200, `score` in response is 1,
    # `message` is "Quiz evaluated successfully", and `mock_evaluate_quiz` was
    # called once
    assert response.status_code == 200
